...
func InitDB(dbPath string) error {
	...
	// 兼容旧表：补充 verify_status 列
	DB.Exec("ALTER TABLE workers ADD COLUMN verify_status TEXT DEFAULT 'none'")
	DB.Exec("ALTER TABLE workers ADD COLUMN verify_note TEXT DEFAULT ''")
...
func migrate() error {
	schema := `
		...
		CREATE TABLE IF NOT EXISTS workers (
			...
			verify_status TEXT DEFAULT 'none' CHECK(verify_status IN ('none','pending','verified','rejected')),
